...
Intercom.prototype.checkAccess = function(action, cb) {
  ...
  if( this.listenerCount('facet:check:access') > 0 ) {
    ...
    // in cases where Gatekeeper is not used but other facet modules are being used
    // allow execution of otherwise restricted access functions but warn in console
    console.warn('WARNING: facet:check:access event was fired but no listeners are registered, allowing execution without access check');
    cb(true);
  }
};
